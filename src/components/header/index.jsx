...
const Header = () => {
  ...
  return (<header className={`header ${scroll ? 'is-fixed' : ''}`}>
          ...
          <div className="col-md-12">
            <div id="site-header-inner">
              ...
              <div className="header-center">
                <nav id="main-nav" className={`main-nav ${menuActive ? 'active' : ''}`}>
                  <ul id="menu-primary-menu" className="menu">
                    {menus.map((data, idx) => (<li key={idx} onClick={() => handleDropdown(idx)}
                                                   className={`menu-item ${data.namesub ? 'menu-item-has-children' : ''} ${activeIndex === idx ? 'active' : ''}`}

                    >
                      <Link to={data.links}>{data.name}</Link>
                      {data.namesub && <ul className="sub-menu">
                        {data.namesub.map((submenu) => (<li key={submenu.id} className="menu-item"><NavLink
              ...
              <div className="header-right">
                {accountStatus ?
                  <Link to="/disconnect-fiba" className="tf-button ">
                    <span>Disconnect</span>
                  </Link> :
                  <Link to="/connect-fiba" className="tf-button ">
                    <span>Connect Fibarium</span>
                  </Link>
                }
                <span className="user">
                  <svg width="20" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <mask id="mask0_2981_49321" maskUnits="userSpaceOnUse" x="0" y="11" width="16" height="7">
                      <path fillRule="evenodd" clipRule="evenodd" d="M0 11.2949H15.1998V18.0009H0V11.2949Z"
                            fill="white"/>
                    </mask>
                    ...
                    <path fillRule="evenodd" clipRule="evenodd" d="M15.8613 10.8457H19.9994V15.5473H15.8613V10.8457Z"
                          fill="white"/>
                    </mask>
              ...
              <div className={`mobile-button ${menuActive ? 'active' : ''}`} onClick={handleMenuActive}><span></span>
              </div>
            </div>
          </div>
        </div>
